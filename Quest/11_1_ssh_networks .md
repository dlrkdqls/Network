...
```
log="$1"
total=$(grep -i "CONNECT" "$log" | wc -l)
success=$(grep -i "success" "$log" | wc -l)
fail=$(grep -i "fail" "$log" | wc -l)
rate=$(echo "scale=2; $success * 100 / $total" | bc)

          echo "Total connection try : $total"
          echo "Sucess : $success"
          echo "Fail : $fail"
...
echo "Connection Frequency Top 3"


ip_list=$(cut -d' ' -f3 "$log" | sort | uniq -c | sort -rn | head -n 3)
...
do
        ...
        first_time=$(grep "$ip" "$log" | head -n1 | cut -d' ' -f2)

  echo "Rank $rank: $ip ($count times)" && \
  echo "First connection time : $first_time"
